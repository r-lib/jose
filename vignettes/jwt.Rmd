...
## Starting v1.1.1 expiration time can be set and check

It can be set at `jwt_encode|decode_*` by passing the argument `duration = <seconds>`. For back compatibilty, parameter is `NULL` by default.

```{r}
# Create and validate 1 minute HMAC tokens
lifetime <- 60
sig <- jwt_encode_hmac(claim, key, duration = lifetime)
jwt_decode_hmac(sig, key, duration = lifetime)
```
